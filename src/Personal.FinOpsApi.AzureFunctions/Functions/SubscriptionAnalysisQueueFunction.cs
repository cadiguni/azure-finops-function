        ...
        ServiceBusReceivedMessage message)
    {
        ...
        try
        {
            ...
            try
            {
                ...
                {
                    _logger.LogInformation("ℹ️ [QUEUE] Tipo de análise {analysisType} não suportado", analysisType);
                }
            }
            ...
            {
                _logger.LogWarning("⏱️ [QUEUE] Timeout detectado - reagendando mensagem: {error}", ex.Message);
                await RescheduleAsync(message, analysisRequest);
                return; // ✅ IMPORTANTE: Não deixar exception estourar
            }
            catch (TimeoutException ex)
            {
                _logger.LogWarning("⏱️ [QUEUE] Timeout de operação - reagendando mensagem: {error}", ex.Message);
                await RescheduleAsync(message, analysisRequest);
                return; // ✅ IMPORTANTE: Não deixar exception estourar
            }
